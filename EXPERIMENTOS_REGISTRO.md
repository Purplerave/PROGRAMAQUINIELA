...
## 2026-08-05 — P0.3 (roadmap auditoría): Extraer `prediction_engine` (core aislado)
- **Objetivo:** separar el núcleo puro de predicción (features → probabilidades
  1/X/2 + lambdas) de la carga de datos, el optimizador de columnas, la decisión
  quinielística y el reporting, de forma que P1.1 (contexto) y futuros
  experimentos puedan iterar sin riesgo de fuga y con interfaz estable.
- **Estado:** IMPLEMENTADO (iso-resultado).
- **Entregables:**
  - `prediction_engine/core.py` — constantes (LABEL_MAP, FEATURE_COLUMNS,
    DIVISION_LABELS), constructores de pipelines (logit, HGB),
    `predict_full_probs`, `hybrid_ensemble`/`apply_hybrid_config`,
    `add_market_baseline`, `build_double`, `top_scorelines`, clase
    `PredictionEngine`.
  - `prediction_engine/training.py` — `simulate_doubles`, `evaluate_config`,
    `summarize_results`, `optimize_hybrid_config` (walk-forward
    multi-temporada) y `train_engine`.
  - `prediction_engine/pleno.py` — Pleno al 15 puro (buckets, top marcadores,
    `add_pleno_al_15`).
  - `MOTOR_QUINIELA_MAESTRO.py` se reduce a fachada: carga de datos
    (`load_raw_history`), CLI (`main`), y funciones de backtest
    (`run_backtest`, `run_season_backtest`, `run_latest_season_backtest`) que
    combinan el core puro con I/O. Re-exporta todos los símbolos públicos
    para que backtests, OPTIMIZADOR_COLUMNAS, MOTOR_PREDICCION_JORNADA y
    PREDECIR_JORNADA sigan funcionando sin cambios.
  - `MOTOR_PREDICCION_JORNADA.py` consume `prediction_engine` directamente
    (dirección de dependencia unidireccional).
  - 7 tests nuevos (`tests/test_prediction_engine_boundaries.py`) que
    verifican: (1) que prediction_engine NO importa de OPTIMIZADOR/DECISION/
    evaluation/reporting/backtests/MOTOR_*; (2) API pública presente;
    (3) compatibilidad de `feature_columns()` con la fachada; (4) smoke test
    de entrenamiento + predicción sobre datos sintéticos.
- **Iso-resultado verificado (05/08/2026):**
  - Backtest walk-forward 2019-2026: 7 temporadas, `accuracy_simple` y
    `mean_hits_3_dobles` **idénticos al bit** (δ=0.000000) para las 7
    temporadas contra `reports/production_reference.json`.
  - Evaluación económica (EVALUACION_ECONOMICA): ROI modelo −50,29%,
    P(≥12)=3,57% (idéntico).
  - EXPERIMENTO_ENSEMBLES_ECONOMICO: mismo veredicto (ningún brazo sustituye).
  - CONSOLIDAR_CALIBRACION: mismo veredicto (0/5, rechazada).
  - **222 tests en verde** (215 preexistentes + 7 nuevos); `git diff --check` limpio.
- **Lección / siguiente paso:** el core puro ya es una función
  `predict(features_pit) → probs+λ`, así que P1.1 (features de contexto:
  rotaciones, fin de temporada, ascenso/descenso, fatiga) puede atacarse sin
  tocar el ensamble ni los flujos de I/O, reduciendo a la mitad el riesgo
  de fuga temporal (que fue el fallo de P1.0).
