...
## Último avance (04/08/2026 — regla anti-sobreconfianza evaluada y NO activada)

- Experimentada la regla "no gastar dobles en partidos con divergencia
  HGB-mercado > 0.10": mejoraba el proxy global (8,63 → 8,65/15) y el
  walk-forward 2023-24/24-25, pero **empeoraba en 2025-26** (proxy −4 aciertos)
  y en los 35 boletos reales de esa temporada (3 dobles 8,06 → 8,03/14).
- Decisión: **NO activar** (config `double_avoid_overconfidence: false`).
  La mejora no era consistente en la temporada de operación real; la auditoría
  de boletos reales refutó una mejora que solo se sostenía en el proxy.
  El código y el experimento quedan documentados y disponibles
  (`EXPERIMENTO_DOBLES_DIVERGENCIA.py`); referencia restaurada 51,64 % / 8,63/15.
